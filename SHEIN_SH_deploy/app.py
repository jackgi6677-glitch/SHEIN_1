...
if st.session_state.current_step == "step1":
    ...
    if st.session_state.excel_columns:
        ...
        if _img_cols_step1:
            st.markdown("---")
            st.subheader("🖼️ 4.5 图片字段飞书映射（可选）")
            st.caption(
                "如果飞书表格中已有图片链接字段，可在此绑定。生成时优先使用手动上传的图片；"
                "若没有上传图片，则自动读取飞书中该字段的链接值填入 Excel。"
            )
            if not is_feishu_ready:
                st.warning("⚠️ 请先在侧栏连接飞书并拉取字段列表，才能绑定图片字段。")
            else:
                for _ic in _img_cols_step1:
                    _ic_cfg = mapping.get(_ic, {})
                    _prev_fk = _ic_cfg.get("feishu_img_key", "")
                    _ic_opts = ["-- 不绑定（仅手动上传）--"] + feishu_fields_list
                    _ic_disp = _prev_fk if _prev_fk in _ic_opts else "-- 不绑定（仅手动上传）--"
                    _ic_idx = _ic_opts.index(_ic_disp) if _ic_disp in _ic_opts else 0
                    _ic_wk = f"feishu_img_bind_{_ic}"
                    if _ic_wk not in st.session_state:
                        st.session_state[_ic_wk] = _ic_opts[_ic_idx]
                    _ic_lbl, _ic_sel = st.columns([1, 2])
                    with _ic_lbl:
                        st.markdown(f"🖼️ **{_display_name(_ic)}**")
                    with _ic_sel:
                        _ic_selected = st.selectbox(
                            "飞书字段（图片链接）",
                            options=_ic_opts,
                            index=_ic_idx,
                            key=_ic_wk,
                            label_visibility="collapsed",
                            help="选择飞书表格中存储该图片链接的字段；未上传本地图时自动引用此链接"
                        )
                    if _ic_selected != "-- 不绑定（仅手动上传）--":
                        mapping.setdefault(_ic, {})["feishu_img_key"] = _ic_selected
                        if not mapping[_ic].get("type"):
                            mapping[_ic]["type"] = "image"
                    else:
                        mapping.setdefault(_ic, {})["feishu_img_key"] = ""
...
# ================================================================
    ...
    # ── 飞书图片预览助手（仅用于 Step 2 图片区）────────────────────────────
    def _fetch_img_preview_record(lookup_val: str) -> dict:
        """按检索值拉取飞书记录，结果按 (lookup_val, match_field) 缓存到 session_state。"""
        if not lookup_val or not _has_auth:
            return {}
        _cache_key = f"s2_img_prev_{_match_field}_{lookup_val}"
        if _cache_key in st.session_state:
            return st.session_state[_cache_key] or {}
        try:
            _tok = get_feishu_tenant_token(_fs_auth["app_id"], _fs_auth["app_secret"])
            _rec = search_feishu_record(
                _tok, _fs_auth["app_token"], _fs_auth["table_id"],
                _match_field, lookup_val
            )
            result = _rec if isinstance(_rec, dict) else {}
        except Exception:
            result = {}
    ...
    if _image_cols:
        ...
        if _use_batch:
            ...
            if not _unique_huohao:
                ...
                _hh_prev_cache = f"s2_img_prev_{_match_field}_{_safe_hh}"
                _batch_prev_rec = st.session_state.get(_hh_prev_cache)
                if _any_hh_feishu:
                    _ph_l, _ph_r = st.columns([4, 1])
                    with _ph_l:
                        if _batch_prev_rec is not None:
                            _n_links = sum(
                                1 for c in _image_cols
                                if _tpl.get(c, {}).get("feishu_img_key")
                                and str(_batch_prev_rec.get(_tpl[c]["feishu_img_key"], "") or "").strip()
                            )
                            st.success(f"✅ 飞书图片预览已加载（{_hh_preview_lookup}），共 {_n_links} 个图片链接")
                        else:
                            st.info("💡 部分图片字段已绑定飞书链接，点击右侧按钮加载预览图片。")
                    with _ph_r:
                        if st.button("🔗 加载飞书图片预览", key=f"btn_hh_prev_{_safe_hh}",
                                     use_container_width=True):
                ...
                _has_sku = bool(_sku_img_cols)
                if _has_shared or _has_sku:
                    _all_batch_flat = _shared_img_cols + _sku_img_cols
                    _caption_parts = []
                    if _has_shared:
                        _caption_parts.append(f"**方块图 / 色块图**（货号 `{_sel_huohao}` 共用）")
                    if _has_sku:
                        _caption_parts.append(f"**SKU图**（每个卖家SKU独立）— 共 {len(_rows_for_hh)} 个SKU")
                    ...
                    for _row_i, (_ri, _rsku) in enumerate(_iter_rows):
                        ...
                        if _has_sku and len(_rows_for_hh) > 1 and _sku_label:
                            st.caption(f"卖家SKU: `{_sku_label}`")
                        _flat_row = st.columns(len(_all_batch_flat))
                        for _j, _img_col in enumerate(_all_batch_flat):
                            _is_shared_col = _img_col in _shared_img_cols
                            with _flat_row[_j]:
                                ...
                                if _is_shared_col:
                                    if _row_i == 0:
                                        ...
                                        if _sh_b:
                                            st.image(_sh_b, use_container_width=True, caption="")
                                        if _batch_prev_rec is not None:
                                            _render_img_feishu_hint(
                                                _img_col, _batch_prev_rec,
                                        ...
                                        if _sh_b2:
                                            st.image(_sh_b2, use_container_width=True, caption="（共用）")
                                        else:
                                            st.caption("— 共用 —")
                                else:
                                    # SKU图：按行独立
            ...
            if _any_single_feishu and _single_lookup:
                _sp_l, _sp_r = st.columns([4, 1])
                with _sp_l:
                    if _single_prev_rec is not None:
                        _n_links_s = sum(
                            1 for c in _image_cols
                            if _tpl.get(c, {}).get("feishu_img_key")
                            and str(_single_prev_rec.get(_tpl[c]["feishu_img_key"], "") or "").strip()
                        )
                        st.success(f"✅ 飞书图片预览已加载（{_single_lookup}），共 {_n_links_s} 个图片链接")
                    else:
                        st.info("💡 部分图片字段已绑定飞书链接，点击右侧按钮加载预览图片。")
                with _sp_r:
                    if st.button("🔗 加载飞书图片预览", key="btn_single_prev",
                                 use_container_width=True):
                        with st.spinner("正在拉取飞书图片预览…"):
            ...
            if _is_multi_variant and _sku_img_cols:
                ...
                for _vi, _vsku in enumerate(_variant_skus):
                    ...
                    if len(_variant_skus) > 1:
                        st.caption(f"卖家SKU: `{_vsku_label}`")
                    _flat_row = st.columns(len(_all_flat_cols))
                    for _j, _img_col in enumerate(_all_flat_cols):
                        _is_shared = _img_col in _shared_img_cols
                        with _flat_row[_j]:
                            st.caption(f"**{_display_name(_img_col)}**")
                            if _is_shared:
                                if _vi == 0:
